# Stanrdard execution for 21 executions
...
tables = [
    ("estabinc1", "execcontrnorm1", "agregnorm1"),
    ("estabinc2", "execcontrnorm2", "agregnorm2"),
    ("estabinc3", "execcontrnorm3", "agregnorm3"),
    ("estabinc4", "execcontrnorm4", "agregnorm4"),
    ("estabinc5", "execcontrnorm5", "agregnorm5"),
    ("estabinc6", "execcontrnorm6", "agregnorm6"),
    ("estabinc7", "execcontrnorm7", "agregnorm7"),
    ("estabinc8", "execcontrnorm8", "agregnorm8"),
]
...
def start_execution(data_exec, control_table):
    """
    Registra o início de uma execução na tabela de controle fornecida.

    :param data_exec: Data de execução
    :param control_table: Nome da tabela de controle (Ex: ExecContrNorm1)
    :return: (cod_ver_exec, time_start) - Código da execução e timestamp de início
    """
    with connect_to_db() as conn:
        with conn.cursor() as cursor:
            ...
            conn.commit()
            print(f"Execução iniciada na tabela {control_table}: COD_VER_EXEC={cod_ver_exec}, TIME_START={time_start}")
            return cod_ver_exec, time_start
...
def run_script(script, data_exec, cod_cube, input_table, agreg_table, control_table):
    """
    ...
    with connect_to_db() as conn:
        try:
            with conn.cursor() as cursor:
                ...
                # Contar registros no SELECT inicial
                count_query = f"SELECT COUNT(*) FROM {input_table} WHERE DATA_INI <= %s AND (DATA_FIM IS NULL OR DATA_FIM > %s)"
                cursor.execute(count_query, (data_exec, data_exec))
                ...
                cursor.execute(script, (data_exec, data_exec))  # Substituir placeholders
...
def save_metrics_to_csv(metrics, cod_ver_exec, data_exec, cod_base, filename="etl_metrics_standard.csv"):
    """
    ...
    for cod_base, (input_table, control_table, agreg_table) in enumerate(tables, start=1):
        ...
        for data_exec_str in datas_execucao:
            try:
                ...
                # Iniciar a execução
                cod_ver_exec, time_start = start_execution(data_exec, control_table)

                # Importar scripts SQL
                from SCRIPTA1 import script1, script2, script3

                # Substituir placeholders com as tabelas atuais
                scripts_with_cod_cube = [
                    (script1.format(input_table=input_table, control_table=control_table, agreg_table=agreg_table), "0001"),
                    (script2.format(input_table=input_table, control_table=control_table, agreg_table=agreg_table), "0002"),
                    (script3.format(input_table=input_table, control_table=control_table, agreg_table=agreg_table), "0003"),
                ]
