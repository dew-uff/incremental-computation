# Scripts run first complete execution to incremental experiment (inc1 and inc2).
# After that, you must choose incremental execution inc1 or inc2.
...
def start_execution(data_exec, control_table):
    ...
    :param control_table: Nome da tabela de controle (Ex: ExecContrinc1)
...
def end_execution(cod_ver_exec, time_start, control_table, agreg_table):
    ...
    :param control_table: Nome da tabela de controle (Ex: ExecContrinc1)
    :param agreg_table: Nome da tabela de agregação (Ex: Agreginc1)
    """
    with connect_to_db() as conn:
        with conn.cursor() as cursor:
...
def save_metrics_to_csv(metrics, cod_ver_exec, data_exec, cod_base, filename="etl_metrics_first_inc.csv"):
    """
    ...
    for cod_base, (input_table, control_table, agreg_table) in enumerate(tables, start=1):
        ...
        for data_exec_str in datas_execucao:
            try:
                ...
                from SCRIPTB1 import script1, script2, script3
